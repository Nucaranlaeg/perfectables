...
bool sort_factors(std::tuple<int, int> a, std::tuple<int, int> b){
	...
	for (int i = 0; i < prime.size(); i++){
		...
		for (int power = 1; power < 32; power++){
			...
			for (int base_power = 1; base_power <= power; base_power++){
				...
				if (sigma == 1){
					std::sort(factor.factors.begin(), factor.factors.begin() + factor.factors.size(), sort_factors);
